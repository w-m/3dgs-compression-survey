...
from build_html import combine_tables_to_html
...
with open("data_extraction/latex/tex_templates/figure.tex", "r") as file:
    tex_figure = file.read()

with open("data_extraction/latex/tex_templates/entry.tex", "r") as file:
    tex_entry = file.read()

with open("data_extraction/latex/tex_templates/table.tex", "r") as file:
    tex_table = file.read()

with open("data_extraction/latex/tex_templates/contributions.tex", "r") as file:
...
_, _, _, _, _, _, table_df_dict = combine_tables_to_html()
multi_col_df_compression = table_df_dict["compression"]
...
for method_i in ["compression", "densification"]:
    multi_col_df = table_df_dict[method_i]
    buffer = io.StringIO()
    ...
    if method_i == "compression":
        ...
        lines[3] = (
            ...
            .replace("LPIPS", "LPIPS$\\downarrow$")
            .replace("Size [MB]", "\\makecell{Size \\\\ MB$\\downarrow$}")
            .replace("#Gaussians", "\#Gaussians$\\downarrow$")
        )

    tex_table_str = "\n".join(lines)

    colors = ["lightred", "lightorange", "lightyellow"]
    tex_table_str = tex_table_str.replace(
        '<td class="first">', "\\cellcolor{" + colors[0] + "}"
    )
    tex_table_str = tex_table_str.replace(
        '<td class="second">', "\\cellcolor{" + colors[1] + "}"
    )
    tex_table_str = tex_table_str.replace(
    ...
    cp_images(src_folder, "../data_extraction/latex/images")

    # copy bib files into latex folder
    shutil.copy2("datasets.bib", "data_extraction/latex/dataset.bib")
    shutil.copy2(
        "methods_densification.bib", "data_extraction/latex/methods_densification.bib"
    )
    shutil.copy2(
        "methods_compression.bib", "data_extraction/latex/methods_compression.bib"
    )
    ...
    # create table
    table = tex_table.replace("<table>", tex_table_str)
    with open(
        "data_extraction/latex/3dgs_table_" + method_i + ".tex", "w", encoding="UTF-8"
    ) as file:
        file.write(table)

os.chdir("data_extraction/latex")
